...
class Authentication_model extends App_Model
{
     ...
     * @param  boolean Force login (for logout from other device confirmation)
     * @return boolean if not redirect url found, if found redirect to the url
     */
    public function login($email, $password, $remember, $staff, $force_login = false)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            // Check if user already logged in from another device
            if (!$force_login && !empty($user->current_session_token)) {
                return [
            ...
            if (!$twoFactorAuth) {
                ...
                // Generate and store session token for single session
                $this->set_session_token($user->$_id, $staff);
            } else {
    ...
    public function logout($staff = true)
    {
        ...
        if (is_client_logged_in()) {
            ...
            // Clear session token
            $this->clear_session_token(get_contact_user_id(), false);
            ...
            // Clear session token
            $this->clear_session_token(get_staff_user_id(), true);
    ...
    public function two_factor_auth_login($user)
    {
        ...
        // Generate and store session token for single session
        $this->set_session_token($user->staffid, true);
     ...
     * @return void
     */
    private function set_session_token($user_id, $staff)
    {
        $table = db_prefix() . 'contacts';
        $_id   = 'id';
        if ($staff == true) {
        ...
        $this->session->set_userdata('current_session_token', $session_token);
     ...
     * @return void
     */
    private function clear_session_token($user_id, $staff)
    {
        $table = db_prefix() . 'contacts';
        $_id   = 'id';
        if ($staff == true) {
