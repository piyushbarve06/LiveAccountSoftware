    ...
    public function admin()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                $force_login = $this->input->post('force_login');

                $data = $this->Authentication_model->login($email, $password, $remember, true, $force_login);

                if (is_array($data) && isset($data['already_logged_in'])) {
                    // Return JSON for AJAX request
                    if ($this->input->is_ajax_request() || $this->input->post('ajax_check')) {
                        $this->output
                            ->set_content_type('application/json')
                            ->set_output(json_encode(['already_logged_in' => true]))
                            ->_display();
                        exit;
                    }
                    set_alert('warning', 'You are already logged in from another device');
                    redirect(admin_url('authentication'));
                } elseif (is_array($data) && isset($data['memberinactive'])) {
